...
export async function listUsers() {
  ...
  // Email login tersimpan di Supabase Auth (auth.users), bukan di
  // profiles — digabung di sini lewat service role supaya Kelola User
  // bisa menampilkan & mengedit email sekaligus (login memakai email,
  // bukan username).
  const { data: authList, error: authError } = await supabaseAdmin.auth.admin.listUsers({ perPage: 1000 });
  if (authError) throw new Error(`Gagal memuat email login: ${authError.message}`);
...
/**
 * Ubah email login akun staf. Login memakai email (bukan username),
 * jadi ini penting supaya Admin bisa memperbaiki salah ketik atau
 * memindahkan akun ke email baru. Butuh service role — mengubah email
 * di Supabase Auth bukan hal yang bisa dilakukan lewat client biasa.
 */
...
/**
 * Hapus akun yang sudah tidak dipakai — menghapus akun Supabase Auth
 * (profiles.id sama dengan auth user id, dan profiles punya
 * "on delete cascade" ke auth.users, jadi baris profil ikut terhapus
 * otomatis). Tidak bisa dibatalkan, jadi konfirmasi dilakukan di UI
 * sebelum action ini dipanggil.
 */
export async function deleteUserAccount(profileId: string) {
  ...
  if (user?.id === profileId) {
    throw new Error("Tidak bisa menghapus akun yang sedang dipakai untuk login saat ini.");
  }
