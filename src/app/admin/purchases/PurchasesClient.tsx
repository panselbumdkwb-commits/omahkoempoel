  ...
  function submitPurchase(fd: FormData) {
    startTransition(async () => {
      try {
        ...
        setMessage("Belanja bahan baku tersimpan.");
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h2 className="font-heading text-2xl sm:text-3xl text-primary">🛒 Belanja Bahan Baku &amp; Aset</h2>
        </div>
        {readOnly && <span className="badge-modern bg-accent/15 text-accent">Mode lihat-saja (Captain)</span>}
      </div>
      ...
      {/* FORM TAMBAH BELANJA HARIAN */}
      {!readOnly && (
        <section className="card-modern p-5">
          <h3 className="section-title-modern mb-3">➕ Catat Belanja Hari Ini</h3>
          <form action={submitPurchase} className="grid grid-cols-2 sm:grid-cols-4 gap-2">
            ...
            <input type="text" name="itemName" placeholder="Nama bahan (mis. Sayur, Kopi)" required className="border border-border rounded-lg p-2 text-sm bg-background dark:bg-background-dark col-span-2 sm:col-span-1" />
            ...
            <input type="text" name="supplier" placeholder="Supplier/Pasar (opsional)" className="border border-border rounded-lg p-2 text-sm bg-background dark:bg-background-dark" />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-text-muted border-b border-border">
                ...
                {!readOnly && <th className="py-1.5"></th>}
              </tr>
            </thead>
            <tbody>
              {purchases.map((p) => (
