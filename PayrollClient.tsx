...
export default function PayrollClient({
  ...
  function runPayroll(fd: FormData) {
    startTransition(async () => {
      try {
        ...
        setPeriodList((cur) => [
          { id: period.id, period_start: period.period_start, period_end: period.period_end, status: period.status, created_at: period.created_at },
          ...cur,
        ]);
      } catch (err: any) {
        setMessage(`Gagal: ${err.message}`);
  ...
  function deletePosition(id: string) {
    if (!confirm("Hapus jabatan ini? Acuan gaji pokoknya akan hilang dari daftar.")) return;
    startTransition(async () => {
      try {
        await deletePositionAction(id);
        setPositionList((cur) => cur.filter((p) => p.id !== id));
      } catch (err: any) {
        setMessage(`Gagal: ${err.message}`);
      }
    });
  ...
  function deletePeriod(id: string) {
    if (!confirm("Hapus periode payroll ini beserta seluruh slip gajinya? Tindakan ini tidak bisa dibatalkan.")) return;
    startTransition(async () => {
      try {
        await deletePayrollPeriodAction(id);
        setPeriodList((cur) => cur.filter((p) => p.id !== id));
        if (selectedPeriod === id) {
          setSelectedPeriod(null);
          setItems(null);
        }
        setMessage("Periode payroll dihapus.");
      } catch (err: any) {
        setMessage(`Gagal: ${err.message}`);
      }
    });
    ...
    <div className="max-w-4xl mx-auto space-y-8">
                  ...
                  action={(fd) =>
                    startTransition(async () => {
                      try {
                        ...
                        setPositionList((cur) =>
                          cur.map((row) =>
                            row.id === p.id ? { ...row, default_basic_salary: Number(fd.get("defaultBasicSalary") ?? 0) } : row
                          )
                        );
      ...
      <section className="card-modern p-5 print:hidden">
        ...
        <div className="divide-y divide-border">
          {periodList.map((p) => (
            <div key={p.id} className="py-2 flex justify-between items-center text-sm gap-2">
              <button onClick={() => openPeriod(p.id)} className="flex-1 text-left hover:text-primary flex justify-between pr-2">
                <span>
              ...
              <button
                ...
                title={p.status === "PAID" ? "Periode PAID tidak bisa dihapus" : "Hapus periode ini"}
                className="text-xs px-2 py-1.5 rounded-md border border-danger/40 text-danger disabled:opacity-40 disabled:cursor-not-allowed"
              >
                Hapus
              </button>
            </div>
          ))}
          {periodList.length === 0 && <p className="text-sm text-text-muted py-2">Belum ada periode payroll.</p>}
        </div>
      </section>
