...
/** Hapus 1 periode payroll (Riwayat Periode Payroll) yang sudah tidak
 * diperlukan, beserta seluruh slip gaji (payroll_items) di dalamnya.
 * Periode berstatus PAID (sudah benar-benar dibayarkan) TIDAK boleh
 * dihapus — itu catatan keuangan yang sudah final, demi jejak audit;
 * hapus/koreksi lewat catatan baru kalau perlu, bukan menghapus riwayat. */
export async function deletePayrollPeriod(periodId: string) {
  ...
  const { data: period, error: fetchError } = await supabase
    ...
    .eq("id", periodId)
    .single();
  if (fetchError || !period) throw new Error("Periode payroll tidak ditemukan.");
  if (period.status === "PAID") {
    throw new Error("Periode yang sudah PAID (dibayarkan) tidak bisa dihapus — ini catatan keuangan final.");
  }

  const { error: itemsError } = await supabase.from("payroll_items").delete().eq("payroll_period_id", periodId);
