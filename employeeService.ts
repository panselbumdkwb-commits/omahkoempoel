...
/** Hapus 1 jabatan (dari "Gaji Pokok per Jabatan" di halaman Payroll)
 * yang sudah tidak diperlukan. Ditolak kalau masih ada pegawai aktif
 * yang memakai jabatan ini — supaya tidak diam-diam kehilangan acuan
 * gaji pokok pegawai yang masih bekerja; pindahkan dulu jabatan
 * pegawai tersebut lewat halaman Pegawai sebelum menghapus. */
export async function deletePosition(id: string) {
  ...
  const { count, error: countError } = await supabase
    ...
    .eq("position_id", id)
    .is("deleted_at", null);
  if (countError) throw new Error(`Gagal memeriksa pemakaian jabatan: ${countError.message}`);
  if (count && count > 0) {
    throw new Error(
      `Jabatan ini masih dipakai ${count} pegawai. Pindahkan jabatan pegawai tersebut dulu di halaman Pegawai sebelum menghapus.`
    );
  }

  const { error } = await supabase.from("employee_positions").delete().eq("id", id);
  if (error) throw new Error(`Gagal menghapus jabatan: ${error.message}`);
}
